...
function mapOpenFoodFactsProduct(product, barcode) {
  ...
  const reason = hasVeganLabel
    ? "Open Food Facts führt das Produkt mit veganem Label."
    : hasNonVeggieSignal
      ? "In den Zutaten wurden Hinweise auf Fleisch, Fisch oder Gelatine gefunden."
      : hasAnimalSignal
        ? "Die Zutaten deuten auf tierische Bestandteile hin. Für vegan bitte genauer prüfen."
  ...
  return {
    ...
    ingredients: ingredientNames.length
      ...
      : [{ name: "Keine Zutatenliste verfügbar", problematic: false }],
    alternative: {
      name: "Vegane Alternative in der Nähe suchen",
      store: "Community-Funde",
      price: "je nach Laden",
      reason: "Als nächster Schritt könnte die App lokale Community-Funde passend zum Produkt anzeigen."
    }
  };
}
