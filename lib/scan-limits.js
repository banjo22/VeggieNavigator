...
export async function getScanQuotaStatus(req) {
  ...
  return quotaResult({ limit, used, usageDate, subjectType: subject.type });
}
...
export async function consumeScanQuota(req) {
  ...
  if (used >= limit) {
    ...
    error.status = 429;
    error.quota = quotaResult({ limit, used, usageDate, subjectType: subject.type });
    throw error;
  }
  ...
  return quotaResult({ limit, used: nextUsed, usageDate, subjectType: subject.type });
}
...
function quotaResult({ limit, used, usageDate, subjectType }) {
  return {
    limit,
    used,
