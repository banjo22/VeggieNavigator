    ...
    return (
      <div className="min-h-screen bg-cream px-5 py-8 text-ink">
        <main className="mx-auto max-w-xl rounded-3xl bg-white p-6 shadow-soft">
          ...
          <p className="mt-3 leading-7 text-ink/65">Ein lokaler Browserzustand oder ein Render-Fehler blockiert gerade die App. Setze die lokalen App-Daten zurueck und lade neu.</p>
  ...
  root.innerHTML = `
    <div style="min-height:100vh;background:#f7f3ea;padding:32px 20px;font-family:Inter,system-ui,sans-serif;color:#242821">
      <main style="max-width:620px;margin:0 auto;background:white;border-radius:24px;padding:24px;box-shadow:0 18px 45px rgba(54,66,49,.12)">
        ...
        <p style="line-height:1.7;color:rgba(36,40,33,.65)">Bitte einmal lokale App-Daten loeschen und neu laden.</p>
        ...
        <button onclick="localStorage.clear();sessionStorage.clear();location.href='/'" style="border:0;border-radius:16px;background:#4f6f52;color:white;padding:12px 18px;font-weight:800">Lokale Daten loeschen</button>
      </main>
    </div>
  `;
}
